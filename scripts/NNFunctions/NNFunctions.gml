...
/// @function		get_leftmost_item(check_fn);
...
function get_leftmost_item(check_fn = undefined) {
	var _leftmost_index = -1;
    
    for (var i = 1; i < array_length(obj_ItemMGMT.ItemInst); i += 1) {
        if (instance_exists(obj_ItemMGMT.ItemInst[i]) && obj_ItemMGMT.ItemInst[i] != -1) {
			if (is_callable(check_fn)) {
				if (!check_fn(obj_ItemMGMT.Item[i])) {
					continue;
				}
			}
	...
	return _leftmost_index;
}

/// @function		get_rightmost_item(check_fn);
/// @param {function}	check_fn	An optional function to check specific attributes of an item.
/// @description	Returns the index of the item where -1 is not in the inventory.
function get_rightmost_item(check_fn = undefined) {
	var _rightmost_index = -1;
    
    for (var i = array_length(obj_ItemMGMT.ItemInst) - 1; i >= 1; i -= 1) {
        if (instance_exists(obj_ItemMGMT.ItemInst[i]) && obj_ItemMGMT.ItemInst[i] != -1) {
			if (is_callable(check_fn)) {
				if (!check_fn(obj_ItemMGMT.Item[i])) {
					continue;
...
/// @function		give_perk(perk_id, [at_x], [at_y]);
...
function give_perk(perk_id, at_x = 950, at_y = 540) {
    var _perk_giver = noone;
	
	if (instance_exists(agi("obj_nnfunctions_perk_giver"))) {
		_perk_giver = instance_find(agi("obj_nnfunctions_perk_giver"), 0);
	} else {
		_perk_giver = instance_create_depth(0, 0, obj_ItemMGMT.depth, agi("obj_nnfunctions_perk_giver"));
	}
...
function give_suspicious_key(at_x = 950, at_y = 540) {
	var _MovingUnq = instance_create_depth(at_x, at_y, obj_ItemMGMT.depth, obj_UnqItemMove);
